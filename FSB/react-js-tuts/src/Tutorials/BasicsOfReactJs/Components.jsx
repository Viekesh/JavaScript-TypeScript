...
// Facebook uses more than 50,000 components and they’re all built using React. This is what makes React, a battle-hardened solution that you can bank on safely.

// Components are composable units that let you focus your energies on building great features. Add declarative rendering, with state and the virtual DOM, and you’ve got yourselves the right toolkit for building great user interfaces and apps.

// Remember everything that you build using is a "Component".

// The actual data comes through the top-level components and gets passed down in a one-way data flow to all the child components.
...
// The bottom line here is, Components are reusable features that you can compose together to create a user interface.
...
// As its most basic, a functional component is a presentational component, which lets you except data through a prop and render it on an UI.

// These functions do not offer built-in state management, lifecycle methods and other bells and whistles that class components offer.
...
// JSX is not HTML. For instance, instead of using an attribute called class, we use className and we can create event listeners like onClick as we saw in the last topic.
...
// Everything that you built using react is a component. Components are building blocks of all react applications that helps you break down a larger interface into smaller units that can be build and maintain independently. Components are reusable features that you can compose together to create a user interface.

// React is one way or unidirectional data flow order, simply means that parent component will pass down data to nested child components using props, which acts like inlet into a component. This unidirectional data flow order makes it easy to understand, implement and debug common problems as recognition of data sources and flow becomes easy.
