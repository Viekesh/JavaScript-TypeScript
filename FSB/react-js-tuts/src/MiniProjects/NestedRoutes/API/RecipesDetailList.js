    ...
    desserts: [
        {
            ...
            description: "These bars mix up quickly. They're moist and delicious with a wonderful cream cheese frosting. They are especially good when chilled in the refrigerator before serving.",
            rating: 5,
            prepTime: 15,
            cookTime: 35,
            ingredients: [
...
export const getCategories = async () => {
    const ctgr = Object.keys(recipes);
    return ctgr;
};

// To fix the error "await has no effect on this type of expression" in the code, you can remove the await
// keyword. This is because the expression Object.keys(recipes) does not return a promise, so there is no need
// to await it.

// Note: It is generally good practice to use await whenever possible when calling functions that return
// promises. This helps to ensure that your code is asynchronous and does not block the main thread. However,
// it is important to remember that await can only be used inside of async functions.
...
export const getRecipes = async (category) => {
    const rcps = await Promise.resolve(recipes[category]);
...
//     const rcp = await Promise.resolve(Object.values(recipes));
//     const recipe = rcp.flat().filter(i => i.id === id);
//     return recipe;
// };