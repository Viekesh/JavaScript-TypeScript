...
// source : https://www.w3schools.com/jsref/tryit.asp?filename=tryjsref_includes2

// learn some JavaScript method before starting this programme :

    // include() :

    // syntax :
    // string.include(searchValue, start/position)
    // searchValue: Required. The string to search for.
    // start/position: Optional. It is the position to start from. Default value is 0.

    // return value :
    // It returns boolean values. "true" if the strings contains the values otherwise it is "false".
...
// it returns if the value contain in a "text" variable at 0th position and it will return false if your
// start position is different and there is nothing any word or character that matches your searchValue.
...
// As with everything in JavaScript, creating a string immediately brings forth a number of methods that exist on 
// the prototype. Using these methods, it is interesting to come up with functions that can help you implement 
// string processing capabilities in your web applications.

// Using String prototype methods you can do things like:

    // detecting the presence of a string in another string
    // targeting and modifying one character at a time, or
    // a character at a given location, using Regular Expressions with text.
    
// Regular expressions, for the uninitiated, are a syntax for defining patterns for matching characters and 
// are found in most languages.
    
// This allows you to build very powerful string-based functions that can help you create and validate known 
// patterns such as email addresses, phone numbers, credit card numbers, and so on. And just like arrays, we 
// can slice and target portions of a string such as extracting data.
...
// Now, we’ll look at a way to find a string in another string. This is useful for creating a search mechanism 
// where you have to scan through pieces of text to find the ones which match. The possibilities are endless. 

// You might remember the Array.includes method we saw earlier, which allows you to confirm the presence of an 
// element in an array.



// string.include() :
    // The "includes()" method is also available on the string prototype which allows us to detect the presence of 
    // a string in another string. This is particularly useful when you have to build a search function that has
    // to scan an array of messages for instance.

    // Strings.includes(searchString, position);

    // You apply this method to the source text, and you provide the string to search for as the first parameter. 
    // There is also an optional second parameter which is the starting position in the source to start searching 
    // from.
...
// Here I have an array with excerpts from two stories, stored as two separate string elements. Now we want to 
// search this array to find and return the element which contains the given string.
...
const findString = (str, arr) => {
    ...
    // scan the array and at each iteration, grab the current element
    let result = arr.filter((elem) =>

        // convert all text to lower case as includes() and performs a case sensitive search
...
// search for ‘secret’ in the stories array
console.log(`ex 2: ${findString('secret', stories)}`);

// search for ‘jack’ in the stories array
console.log(`ex 2: ${findString('jack', stories)}`);

// search for ‘vector’ in the stories array
console.log(`ex 2: ${findString('vector', stories)}`);
